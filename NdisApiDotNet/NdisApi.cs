...
namespace NdisApiDotNet
{
    public unsafe class NdisApi : IDisposable
    {
        ...
        {
            buffer->m_dwDeviceFlags = 0;
            buffer->m_Flags = 0;
            buffer->m_qLink_hAdapter.m_hAdapter = IntPtr.Zero;
            buffer->m_8021q = 0;
            buffer->m_FilterID = 0;
            buffer->m_Length = 0;

            var reservedLength = (int) Native.NdisApi.INTERMEDIATE_BUFFER.BufferOffset - (int) Native.NdisApi.INTERMEDIATE_BUFFER.ReservedOffset;

            Kernel32.ZeroMemory((IntPtr) buffer + (int) Native.NdisApi.INTERMEDIATE_BUFFER.ReservedOffset, reservedLength);
            Kernel32.ZeroMemory((IntPtr) buffer + (int) Native.NdisApi.INTERMEDIATE_BUFFER.BufferOffset, MaxPacketSize);
        }
