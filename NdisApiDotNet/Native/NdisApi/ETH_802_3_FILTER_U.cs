...
namespace NdisApiDotNet.Native
{
    ...
    public unsafe struct Eth802_3FilterUnsafe // ETH_802_3_FILTER_U
    {
        internal FilterFlags m_ValidFields;
        internal fixed byte m_SrcAddress[NdisApi.ETHER_ADDR_LENGTH];
        internal fixed byte m_DestAddress[NdisApi.ETHER_ADDR_LENGTH];
        ...
        /// </summary>
        public FilterFlags ValidFields
        {
            get
            {
        ...
        {
            byte[] bytes = new byte[NdisApi.ETHER_ADDR_LENGTH];

            fixed (byte* b = m_SrcAddress)
            {
                Marshal.Copy((IntPtr)b, bytes, 0, NdisApi.ETHER_ADDR_LENGTH);
            }
        ...
        {
            for (int i = 0; i < NdisApi.ETHER_ADDR_LENGTH; i++) m_SrcAddress[i] = bytes[i];
        }
        ...
        {
            for (int i = 0; i < NdisApi.ETHER_ADDR_LENGTH; i++) m_DestAddress[i] = bytes[i];
        }
    }
}